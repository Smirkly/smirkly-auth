...
SmtpEmailVerificationSender::SmtpEmailVerificationSender(
  ...
  if (!smtp_sender_)
    throw std::runtime_error(
        "SmtpEmailVerificationSender: smtp_sender is null");
}

void SmtpEmailVerificationSender::SendVerificationEmail(
    const services::ports::VerificationEmail& msg) {
  const auto result =
      smtp_sender_->SendVerificationCode(msg.to_email, msg.code);

  if (!result.ok) {
    std::ostringstream oss;
    oss << "SMTP send failed" << " kind=" << static_cast<int>(result.kind)
        << " retryable=" << (result.retryable ? 1 : 0)
        << " error=" << result.error
        << " correlation_id=" << msg.correlation_id;
    throw services::ports::EmailDeliveryError(oss.str(), result.retryable);
  }
}

void SmtpEmailVerificationSender::SendPasswordResetEmail(
    const services::ports::PasswordResetEmail& msg) {
  const auto result =
      smtp_sender_->SendPasswordResetToken(msg.to_email, msg.token);
