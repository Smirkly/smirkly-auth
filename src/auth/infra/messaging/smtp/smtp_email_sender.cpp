...
std::string ExtractDomain(std::string_view email) {
  ...
  if (at == std::string_view::npos || at + 1 >= email.size())
    return "localhost";
...
std::string MakeMessageId(std::string_view from_email) {
  return "<" + RandomHex(16) + "." + RandomHex(8) + "@" +
         ExtractDomain(from_email) + ">";
}

std::string FormatMailbox(std::string_view email,
                          std::string_view display_name) {
  // Conservative display name formatting: avoid RFC2047 encoding/quoting
  // complexity.
...
smtp::Rfc822Message BuildPlaintextRfc822(std::string_view from_email,
...
SmtpEmailSender::SmtpEmailSender(smtp::SmtpConfig smtp_config,
...
smtp::SmtpSendResult SmtpEmailSender::SendPlaintext(const std::string& to_email,
                                                    const std::string& subject,
                                                    const std::string& body) {
  if (from_email_.empty()) {
    return {false, false, smtp::SmtpErrorKind::kProtocol,
            "From email is empty"};
  }
  if (to_email.empty()) {
    return {false, false, smtp::SmtpErrorKind::kProtocol, "To email is empty"};
  }
  if (ContainsCrlf(from_email_) || ContainsCrlf(to_email) ||
      ContainsCrlf(subject) || ContainsCrlf(from_name_)) {
    return {false, false, smtp::SmtpErrorKind::kProtocol,
            "CRLF in headers is not allowed"};
  }

  const auto msg =
      BuildPlaintextRfc822(from_email_, from_name_, to_email, subject, body);
...
smtp::SmtpSendResult SmtpEmailSender::SendVerificationCode(
    const std::string& to_email, const std::string& code) {
  const std::string subject = "Smirkly verification code";
  const std::string body = "Your verification code: " + code +
                           "\r\n\r\n"
...
smtp::SmtpSendResult SmtpEmailSender::SendPasswordResetToken(
    const std::string& to_email, const std::string& token) {
  const std::string subject = "Smirkly password reset";
  const std::string body = "Your password reset token: " + token +
                           "\r\n\r\n"
                           "If you didn't request it, ignore this message.\r\n";
