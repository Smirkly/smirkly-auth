...
std::optional<ParsedIp> ParseIp(std::string_view raw) {
  ...
  if (::inet_pton(AF_INET, value.c_str(), &ipv4) == 1) {
    ParsedIp parsed{.version = IpVersion::kV4,
                    .text = InetToString(AF_INET, &ipv4)};
...
std::optional<std::uint8_t> ParsePrefixLength(std::string_view raw,
                                              IpVersion version) {
  raw = Trim(raw);
  if (raw.empty()) {
    return std::nullopt;
...
bool PrefixMatches(const std::array<unsigned char, 16>& network,
...
std::optional<std::vector<ParsedIp>> ParseForwardedFor(
...
IpNetwork ParseTrustedProxyCidr(std::string_view cidr) {
  ...
  const auto address_part =
      slash_pos == std::string_view::npos ? cidr : cidr.substr(0, slash_pos);
    ...
    return std::any_of(trusted_proxies.begin(), trusted_proxies.end(),
                       [&address](const IpNetwork& network) {
                         return network.version == address.version &&
                                PrefixMatches(network.bytes, address.bytes,
...
ClientIpExtractor& ClientIpExtractor::operator=(ClientIpExtractor&&) noexcept =
    default;
...
services::contracts::RequestMeta RequestMetaExtractor::Extract(
  ...
  if (const auto user_agent = request.GetHeader("User-Agent");
      !user_agent.empty()) {
