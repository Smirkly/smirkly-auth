...
inline domain::Session MakeSession(std::string id, std::string user_id,
...
inline domain::User MakeUser(std::string id, std::string password_hash) {
  const auto now = std::chrono::system_clock::now();
...
inline ports::EmailVerification MakeEmailVerification(
    std::string id, std::string user_id, std::string code_hash,
